    ...
    <div style="margin-bottom:32px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px;">
            ...
            <div class="card" style="position:relative;">
                ...
                <div style="position:absolute;top:12px;right:12px;background:var(--gold-dim);color:var(--gold);font-size:10px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;padding:3px 8px;border-radius:100px;">Built-in</div>
                @endif
                <div style="padding:20px 20px 16px;">
                    ...
                    <p style="font-size:12px;color:var(--muted);margin-bottom:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $template->subject }}</p>
                    <div style="font-size:11px;color:var(--muted);">Updated {{ $template->updated_at->diffForHumans() }}</div>
                </div>
                <div style="border-top:1px solid var(--border);padding:12px 20px;display:flex;gap:8px;align-items:center;">
                    ...
                    <form method="POST" action="{{ route('templates.destroy', $template) }}" style="margin-left:auto;"
...
<div class="card" style="text-align:center;padding:60px 24px;">
    ...
    <div style="display:flex;justify-content:center;gap:12px;flex-wrap:wrap;">
        ...
        <a href="{{ route('templates.seed') }}" class="btn btn-secondary">✨ Load Built-in Templates</a>
    </div>
</div>
@else
